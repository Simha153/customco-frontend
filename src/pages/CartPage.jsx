...
// Helper to extract price from cart item (handles multiple response shapes)
const getPrice = (item) =>
  item.unitPrice || item.price || item.product?.price ||
  item.productPrice || item.itemPrice || 0
  ...
  return (
    <div style={{ background:'#F5F0E8', minHeight:'80vh', padding:'24px 16px' }}>
      <style>{`
        ...
        .cart-header-row { display:grid; grid-template-columns:1fr 100px 80px 40px; gap:12px; padding:10px 16px; border-bottom:1px solid rgba(28,28,24,0.1) }
        .cart-item-row { display:grid; grid-template-columns:1fr 100px 80px 40px; gap:12px; padding:16px; background:#fff; border-bottom:0.5px solid rgba(28,28,24,0.06); align-items:center }
        @media(max-width:700px){
          .cart-layout { grid-template-columns:1fr!important }
          .cart-header-row { display:none!important }
          .cart-item-row { grid-template-columns:1fr!important; gap:10px!important }
        }
      `}</style>
              ...
              return (
                <div key={item.id} className="cart-item-row">
                  <div style={{ display:'flex', gap:'12px', alignItems:'center' }}>
                    <div style={{ width:'52px', height:'52px', background:bgs[idx%bgs.length], display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', color:'rgba(255,255,255,0.15)', flexShrink:0 }}>👔</div>
                    <div>
          ...
          <div style={{ background:'#fff', border:'0.5px solid rgba(28,28,24,0.08)' }}>
            ...
            <div style={{ padding:'24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'sans-serif', fontSize:'13px', color:'#8C8C88', marginBottom:'10px' }}>
                <span>Subtotal ({cart.length} items)</span><span>₹{subtotal.toLocaleString('en-IN')}</span>
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'sans-serif', fontSize:'13px', color:'#8C8C88', marginBottom:'10px' }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', fontFamily:'sans-serif', fontSize:'13px', color:'#8C8C88', marginBottom:'10px', paddingBottom:'16px', borderBottom:'1px solid rgba(28,28,24,0.08)' }}>
                <span>Shipping</span><span style={{ color:G, fontWeight:600 }}>Free</span>
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:'18px', color:'#1A1A18', marginBottom:'24px', fontWeight:500 }}>
                <span>Total</span><span style={{ color:C }}>₹{total.toLocaleString('en-IN')}</span>
              </div>
              <button onClick={placeOrder} disabled={placing}
                style={{ width:'100%', background:placing?'#8C8C88':P, color:'#fff', border:'none', padding:'14px', fontFamily:'sans-serif', fontSize:'10px', letterSpacing:'0.18em', textTransform:'uppercase', fontWeight:700, cursor:placing?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
